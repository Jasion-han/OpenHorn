...
function parseMessage(msg: ApiMessage): Message {
  ...
  const parseAttachmentsMeta = (value: unknown): Message["attachmentsMeta"] | undefined => {
    if (!Array.isArray(value)) return undefined;
    const out: NonNullable<Message["attachmentsMeta"]> = [];
  ...
  return {
    ...
    attachmentsMeta: parseAttachmentsMeta(
      (msg as unknown as { attachmentsMeta?: unknown }).attachmentsMeta,
    ),
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  setConversationModel: async (
    ...
    set((s) => ({
      conversations: s.conversations.map((c) =>
        c.id === conversationId
          ? { ...c, channelId: channelId || undefined, modelId: modelId || undefined }
          : c,
      ),
      currentConversation:
    ...
    try {
      ...
      if (prev) {
        set((s) => ({
          conversations: s.conversations.map((c) =>
            c.id === conversationId
              ? { ...c, channelId: prev.channelId, modelId: prev.modelId }
              : c,
          ),
          currentConversation:
            s.currentConversation?.id === conversationId
  ...
  appendMessageDelta: (id, delta, updates) =>
    set((state) => ({
  ...
  updateMessage: (id, content, updates) =>
    set((state) => ({
      messages: state.messages.map((m) => (m.id === id ? { ...m, content, ...updates } : m)),
    })),
