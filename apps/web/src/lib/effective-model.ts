  ...
  if (conversation?.channelId && conversation.modelId) {
    ...
    if (!channel) {
      return {
        ok: false,
        scope: "conversation",
        reason: "该对话选择的渠道不存在或已被删除，请重新选择模型。",
      };
    }
    if (!channel.enabled) {
      return {
    ...
    if (!model) {
      return {
        ok: false,
        scope: "conversation",
        reason: "该对话选择的模型不存在或已被移除，请重新选择模型。",
      };
    }
    if (!model.enabled) {
      return {
  ...
  if (!def) {
    ...
    if (hasConversationOverride) {
      return {
        ok: false,
        scope: "conversation",
        reason: "该对话的模型设置不完整或不可用，请重新选择模型。",
      };
    }
    return {
      ok: false,
      scope: "global",
      reason: "未配置可用的默认渠道/默认模型，请先在设置里完成配置。",
    };
  }
