...
function isLikelyBrowserFetchFailure(message: string): boolean {
  ...
  // Browser/network failures often surface as exactly "Failed to fetch" (or with a "TypeError:" prefix).
  // Do NOT treat longer provider/backend error messages like "Failed to fetch models (404)" as backend-down.
