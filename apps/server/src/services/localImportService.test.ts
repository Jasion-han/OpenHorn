...
import { conversations, importRecords, messages, projects, settings, users } from "db";
 ...
 */
async function materializeHome(): Promise<string> {
  ...
  for (const [from, to] of [
    ...
    await cp(path.join(FIXTURE_HOME, from), path.join(home, to), {
      recursive: true,
...
test("claude: linearizes user/assistant text, merges multi-line assistant by message.id, drops meta/tool/compact/error lines", async () => {
  const userId = await seedUser();
  try {
    const project = {
    ...
    expect(convPart?.note).toBe("共 5 条消息");
    ...
    expect(importedItem?.detail).toBe("5 条消息 · /Users/test/Project/alpha");
...
test("claude: re-run is idempotent (skip when unchanged, re-import messages when the source grew)", async () => {
  const userId = await seedUser();
  ...
  try {
    ...
    expect(secondPart?.skipped).toBe(1);
    expect(secondPart?.items[0].link).toEqual({ kind: "conversation", id: CLAUDE_SESSION });
    expect(await loadMessages(CLAUDE_SESSION)).toHaveLength(5);
    expect(
    ...
    expect(thirdPart?.items[0].detail?.startsWith("已更新 · 7 条消息")).toBe(true);
    expect(thirdPart?.note).toBe("共 7 条消息");
    const rows = await loadMessages(CLAUDE_SESSION);
    expect(rows).toHaveLength(8);
    expect(rows[6].content).toBe("Sure.");
    expect(rows[7].id).toBe("openhorn-authored");
...
test("codex: drops developer/injected/commentary, keeps final answers with model + usage, skips openhorn-originated threads", async () => {
  ...
  try {
    ...
    expect(rows.map((m) => [m.role, m.content])).toEqual([
      ...
      ["user", "[Image #1] make it return json"],
