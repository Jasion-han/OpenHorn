...
// Short, stable fingerprint of the resolved channel's credentials + endpoint + protocol.
// Folding this into the cache key means editing a channel's apiKey/baseUrl (or switching
...
function getCompatibilityCacheKey(
  ...
  return `${userId}:${channelId}:${modelId.trim()}:${identityFingerprint}`;
}
...
export async function checkChannelAgentCompatibility(
  ...
  const cacheKey = getCompatibilityCacheKey(
    ...
    channelId,
    trimmedModelId,
    computeChannelIdentityFingerprint(apiKey, baseUrl || "", protocol),
  );
  ...
  const probePromise = (async (): Promise<AgentCheckResult> => {
    ...
    // Other native tool-calling protocols (e.g. google/gemini) whose adapter implements
    // structured tool calling are probed via the generic tool-calling path, mirroring `openai`.
    if (adapterSupportsToolCalling(createAdapter(protocol, apiKey, baseUrl))) {
      const result = await probeGenericToolCallingCompatibility({
        apiKey,
        modelId: trimmedModelId,
        baseUrl,
    ...
    // Last resort for unknown protocols without a tool-calling adapter: the Claude SDK path.
