...
test('stream chat emits live status metadata before assistant deltas', async () => {
  ...
  const chatStreamCalls: Array<{ model: string; messages: Array<{ role: string; content: unknown }> }> = [];
  ...
  mock.module('../agent-adapters', () => ({
    createAdapter: () => ({
      chatStream: async function* (input: { model: string; messages: Array<{ role: string; content: unknown }> }) {
