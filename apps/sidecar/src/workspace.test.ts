...
describe('workspace path safety', () => {
  test('rejects absolute path', async () => {
    const workspaceRoot = await canonicalizeWorkspaceRoot(mkdtempSync(path.join(os.tmpdir(), 'openhorn-ws-')));
    expect(() => resolvePathInsideWorkspace({ workspaceRoot, targetPath: '/etc/passwd' })).toThrow();
  });
