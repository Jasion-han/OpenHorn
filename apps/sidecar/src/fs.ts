...
export async function fsList(input: {
  ...
  const resolvedDir = resolvePathInsideWorkspace({ workspaceRoot: input.workspaceRoot, targetPath: input.dir });
  await assertExistingPathInsideWorkspace({ workspaceRoot: input.workspaceRoot, resolvedPath: resolvedDir });
...
export async function fsReadText(input: {
  ...
  const resolved = resolvePathInsideWorkspace({ workspaceRoot: input.workspaceRoot, targetPath: input.filePath });
  await assertExistingPathInsideWorkspace({ workspaceRoot: input.workspaceRoot, resolvedPath: resolved });
